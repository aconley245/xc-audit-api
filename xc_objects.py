...
class XC_Namespace:
    ...
    def get_waf_policies(self):
        ...
        for waf_policy in bulk_waf_policies.json()["items"]:
            waf_policies.append({"name": waf_policy["name"], "namespace": {"name": self.name}})
...
class XC_HTTP_LB:
    ...
    def get_waf_policy(self):
        '''Queries the LB attibutes to get the name of the Application Firewall applied to the HTTP LB'''
